...
// Poll until succeed/failed — returns video URL on success
...
function makeActionShortFilmTemplate() {
  ...
  const eHero    = { id:`e_${uid()}`, kind:"character", name:"Alex Mercer",  tag:"@hero",    description:"Field operative, mid-30s, black tactical clothing, intense focus", notes:"Protagonist", _imgUrl:"", assetId:"" };
  ...
  const eDrive   = { id:`e_${uid()}`, kind:"object",    name:"Encrypted Drive",tag:"@drive",   description:"Small black biometric case containing classified data",           notes:"MacGuffin",  _imgUrl:"", assetId:"" };
  const eBunker  = { id:`e_${uid()}`, kind:"location",  name:"Control Room",  tag:"@bunker",  description:"Underground government bunker, dim red lighting, monitors everywhere", notes:"",       _imgUrl:"", assetId:"" };
...
function makeTrapMusicVideoTemplate() {
  // ── Bible entities ────────────────────────────────────────────────────────────
  const eArtist  = { id:`e_${uid()}`, kind:"character", name:"The Artist",       tag:"@artist",  description:"The rap artist. Hoodie up or off — always commanding the frame. Jewellery hot. Eyes communicate before the mouth does.", notes:"Primary talent — generate reference portrait first, save to bible before generating scenes", _imgUrl:"", assetId:"" };
  ...
  const eChain   = { id:`e_${uid()}`, kind:"object",    name:"Chain",             tag:"@chain",   description:"Thick cuban link gold chain, worn close to the collarbone. Heavy. Real. Every shot where @artist is above medium should show it.", notes:"Key product detail in all scenes", _imgUrl:"", assetId:"" };
  const eBlock   = { id:`e_${uid()}`, kind:"location",  name:"The Block",         tag:"@block",   description:"Corner of a residential urban street, night. Amber sodium lamps, concrete curb, a mix of steel shutters and old brick. The block that raised everyone here.", notes:"Primary location — cold open and street scenes", _imgUrl:"", assetId:"" };
